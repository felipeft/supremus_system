...
export default function Pronto() {
  ...
  const fetchDados = () => {
    ...
    fetch(`${API_URL}/listar-os`).then(res => res.json()).then(data => { 
      // FILTRO: Apenas os que estão no status Pronto
  ...
  const handleSalvarEdicao = async (id, dadosEditados) => {
    try {
      ...
      if (res.ok) {
        alert("✅ SUCESSO: Status atualizado! O equipamento será movido se o status mudou.");
  ...
  return (
    <div className="space-y-6 print:m-0 print:p-0">
      <div className="bg-white shadow-xl rounded-2xl overflow-hidden border border-gray-200 print:hidden">
        ...
        {listaOS.length === 0 && <div className="p-10 text-center text-gray-400 font-bold uppercase text-xs tracking-widest">Nenhum equipamento pronto no momento</div>}
      </div>
      ...
      <div className="hidden print:block bg-white w-full h-full p-10 text-[#24414d]">
        <div className="flex justify-between items-start border-b-2 border-gray-100 pb-6 mb-6">
           <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-xl font-black uppercase">Service Soluções</h1>
                <p className="text-[10px] text-gray-500 uppercase leading-tight">Av. Conselheiro Gomes de Freitas, 3344 - SALA - 203</p>
              </div>
           </div>
           <div className="text-right">
              <h2 className="text-lg font-black text-blue-800">ORDEM DE SERVIÇO</h2>
              <p className="text-xs font-bold uppercase">DATA: {new Date().toLocaleDateString('pt-BR')}</p>
           </div>
        </div>
        <div className="grid grid-cols-2 gap-8 mb-8">
           <div>
              <p className="text-[10px] font-black text-gray-400 uppercase">A/C Cliente</p>
              <p className="text-lg font-black">{os.cliente}</p>
              <p className="text-sm font-mono">{os.whatsapp}</p>
           </div>
           <div className="text-right">
              <p className="text-[10px] font-black text-gray-400 uppercase">Número da OS</p>
              <p className="text-3xl font-black">#{os.os}</p>
           </div>
        </div>
        <div className="space-y-6">
          <div className="border-l-4 border-[#24414d] pl-4">
            <h3 className="text-xs font-black uppercase text-gray-400 mb-1">Equipamento</h3>
            <p className="text-md font-bold uppercase">{os.equipamento}</p>
            <p className="text-xs text-gray-500">NS: {os.numeroSerie} | Acessórios: {os.acessorios}</p>
          </div>
          <div className="bg-gray-50 p-6 rounded-2xl">
            <h3 className="text-xs font-black uppercase text-blue-800 mb-2">Diagnóstico Técnico</h3>
            <p className="text-sm leading-relaxed whitespace-pre-wrap">{form.diagnostico_tecnico}</p>
          </div>
          <div className="flex justify-between items-center bg-yellow-50 p-6 rounded-2xl border border-yellow-200">
            <div>
              <p className="text-[10px] font-black text-yellow-700 uppercase">Valor do Serviço</p>
              <p className="text-3xl font-black">R$: {form.valor_reparo}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white w-full max-w-5xl rounded-[2.5rem] shadow-2xl flex flex-col my-auto border-t-8 border-[#24414d] overflow-hidden print:hidden">
        <div className="p-6 bg-gray-50 border-b flex justify-between items-center text-[#24414d]">
          ...
          <div className="flex gap-2">
            <button onClick={() => window.print()} disabled={!podeImprimir} className={`p-3 rounded-xl flex items-center gap-2 font-black text-xs uppercase transition-all ${podeImprimir ? 'bg-blue-600 text-white shadow-lg' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}>
        ...
        <div className="p-8 border-t bg-gray-50 flex justify-end">
          <button onClick={() => onSave(form)} className="px-12 py-4 bg-[#f8e309] text-[#24414d] font-black rounded-2xl shadow-xl flex items-center gap-2 transition-all hover:scale-105 active:scale-95"><Save size={20} /> SALVAR ALTERAÇÕES</button>
        </div>
      </div>
    </div>
  );
