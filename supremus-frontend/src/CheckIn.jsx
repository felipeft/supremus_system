...
// IMPORTAÇÕES DOS MÓDULOS (Certifique-se de que os caminhos estão corretos)
...
export default function CheckIn() {
  ...
  const handleImprimir = () => {
    const osFinal = osGerada || obterOSBase();
    // A prioridade não é enviada para o PDF, conforme solicitado
    gerarReciboPDF(formData, selectedCountry, acessorios, osFinal);
  };
  ...
  const handleSalvar = async (e) => {
    ...
    try {
      ...
      if (response.ok) { const resultado = await response.json(); setOsGerada(resultado.os); alert(`✅ EQUIPAMENTO REGISTRADO!\nOS Gerada: ${resultado.os}`); }
    ...
    <div className="max-w-2xl mx-auto bg-white shadow-xl rounded-3xl overflow-hidden border border-gray-200 font-sans">
      ...
      <form onSubmit={handleSalvar} className="p-6 space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-2 items-stretch">
            ...
            <div className={`relative flex-1 border-2 rounded-xl transition-colors ${phoneStatusColor}`}>
              <input type="text" placeholder="Número" value={formData.whatsapp} required className="w-full h-full p-3 bg-transparent outline-none text-sm font-mono rounded-xl" onChange={handlePhoneChange} />
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4 border-t pt-4 text-[#24414d]">
          ...
          <div className="grid grid-cols-2 gap-4">
            <input type="text" placeholder="Marca" required value={formData.marca} className="w-full p-3 bg-gray-50 border border-gray-100 rounded-xl outline-none focus:border-[#24414d] text-sm" onChange={(e) => setFormData({...formData, marca: e.target.value})} />
            <input type="text" placeholder="Modelo" required value={formData.modelo} className="w-full p-3 bg-gray-50 border border-gray-100 rounded-xl outline-none focus:border-[#24414d] text-sm" onChange={(e) => setFormData({...formData, modelo: e.target.value})} />
          </div>
          <input type="text" placeholder="Número de Série (NS)" required value={formData.numeroSerie} className="w-full p-3 bg-gray-50 border border-gray-100 rounded-xl outline-none focus:border-[#24414d] text-sm" onChange={(e) => setFormData({...formData, numeroSerie: e.target.value})} />
        </div>
        ...
        {/* Relato */}
        <textarea placeholder="O que o cliente relatou?" required value={formData.relato} className="w-full p-3 bg-gray-50 border border-gray-100 rounded-xl outline-none min-h-[100px] text-sm focus:border-[#24414d]" onChange={(e) => setFormData({...formData, relato: e.target.value})}></textarea>

        {/* Prazo / Prioridade (Logica Interna Restaurada) */}
        ...
        <div className="grid grid-cols-2 gap-4 pt-4">
          <button type="submit" className="bg-[#24414d] hover:bg-[#1a313a] text-white font-bold py-4 rounded-2xl shadow-lg flex items-center justify-center gap-2 active:scale-95 transition-all text-sm uppercase">
            <Save size={18} /> Registrar OS
          </button>
          <button type="button" onClick={handleImprimir} className="bg-gray-100 hover:bg-gray-200 text-[#24414d] font-bold py-4 rounded-2xl flex items-center justify-center gap-2 text-sm border border-gray-200 uppercase">
