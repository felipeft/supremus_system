...
import { Smartphone, User, ClipboardList, Save, Hash, Search, ChevronDown, CheckCircle2, AlertTriangle, Loader2, Printer } from 'lucide-react';
...
const applyMask = (digits, mask) => {
  ...
  for (let i = 0; i < mask.length && dIdx < digits.length; i++) {
    mask[i] === '#' ? result += digits[dIdx++] : result += mask[i];
  }
  return result;
};

const isPhoneValid = (digits, country) => 
...
function CountrySelector({ selected, onChange, disabled }) {
  ...
  return (
    <div className={`relative flex-none w-44 ${disabled ? 'opacity-50 pointer-events-none' : ''}`} ref={dropdownRef}>
      <button type="button" onClick={() => setOpen(!open)} className="w-full h-full p-3 bg-gray-50 border border-gray-100 rounded-xl text-sm font-bold flex items-center gap-2 hover:bg-gray-100 transition-colors">
      ...
      {open && (
        <div className="absolute z-50 top-full left-0 mt-1 w-72 bg-white border border-gray-200 rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div className="max-h-56 overflow-y-auto">
            {filtered.map(c => (
              <button key={c.code + c.name} type="button" onClick={() => { onChange(c); setOpen(false); setSearch(''); }} className={`w-full px-4 py-2.5 flex items-center gap-3 hover:bg-gray-50 text-left ${selected.code === c.code ? 'bg-[#24414d]/5' : ''}`}>
...
// Estados de controle transacional e dirty-checking
  ...
  const handlePhoneChange = (e) => {
    ...
    setPhoneDigits(raw);
    setFormData(prev => ({ ...prev, whatsapp: applyMask(raw, selectedCountry.mask) }));
    setFormModificado(true);
  };

  const handleCheckbox = (e) => { 
    setAcessorios(prev => ({ ...prev, [e.target.name]: e.target.checked })); 
    setFormModificado(true);
  };
...
// Fluxo atômico: Persistência no DB seguida de gatilho automático para PDF
  const handleSalvarERecibo = async (e) => {
    e.preventDefault();
    if (estaSalvando || !phoneValid) return;

    setEstaSalvando(true);
    const listaAcessorios = Object.keys(acessorios).filter(k => acessorios[k]).map(k => k.replace('_', ' ').toUpperCase()).join(", ");
    const payload = { ...formData, whatsapp: `${selectedCountry.code} ${formData.whatsapp}`, ddi: selectedCountry.code, acessorios: listaAcessorios || "Nenhum" };
    ...
    try {
      ...
      if (response.ok) {
        ...
        await gerarReciboPDF(formData, selectedCountry, acessorios, os);
        alert(`✅ SUCESSO: Equipamento registrado sob a OS #${os}`);
      }
    } catch (error) {
    ...
    <div className="max-w-2xl mx-auto bg-white shadow-xl rounded-3xl overflow-hidden border border-gray-200 font-sans">
      ...
      <form onSubmit={handleSalvarERecibo} className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4 pt-4">
          <button type="submit" disabled={estaSalvando} className={`font-bold py-4 rounded-2xl shadow-lg flex items-center justify-center gap-2 active:scale-95 transition-all text-sm uppercase ${estaSalvando ? 'bg-gray-400 cursor-not-allowed text-white' : 'bg-[#24414d] hover:bg-[#1a313a] text-white'}`}>
            {estaSalvando ? <><Loader2 className="animate-spin" size={18} /> Salvando...</> : <><Save size={18} /> Registrar OS</>}
          </button>
          
          <button type="button" onClick={handleImprimirNovamente} className={`font-bold py-4 rounded-2xl flex items-center justify-center gap-2 text-sm border uppercase transition-all ${osGerada && !formModificado && !estaSalvando ? 'bg-gray-100 hover:bg-gray-200 text-[#24414d] border-gray-200' : 'bg-gray-50 text-gray-300 border-gray-100 cursor-not-allowed opacity-30'}`}>
        ...
        {osGerada && !estaSalvando && (
          <div className={`mt-4 p-4 rounded-2xl text-center border animate-in fade-in ${formModificado ? 'bg-orange-50 border-orange-200' : 'bg-yellow-50 border-yellow-200'}`}>
            <p className="text-[#24414d] font-black text-sm uppercase">
              {formModificado ? '⚠️ Dados alterados - Salve novamente' : `Equipamento Registrado! #${osGerada}`}
            </p>
          </div>
        )}
      </form>
